...
const ProjectCard: React.FC<{ project: Project }> = ({ project }) => {
    ...
    return (
        <div 
            ...
            className={`lorris-card rounded-xl overflow-hidden flex flex-col h-full ${!isPrimary ? 'opacity-85 hover:opacity-100' : ''}`}
            style={{ transform, transition: 'transform 0.3s ease-out' }}
        >
            <div className={`overflow-hidden bg-gradient-to-br from-green-accent/5 to-transparent ${isPrimary ? 'h-44' : 'h-32'}`}>
            ...
            <div className={`flex flex-col flex-grow ${isPrimary ? 'p-5' : 'p-4'}`}>
                <h3 className={`font-display font-semibold text-neutral-100 mb-2 line-clamp-2 ${isPrimary ? 'text-base min-h-[2.5rem]' : 'text-sm min-h-[2rem]'}`}>
                    {project.title.replace(/-/g, ' ').replace(/_/g, ' ')}
                </h3>
                <p className={`text-neutral-500 mb-4 flex-grow leading-relaxed line-clamp-3 ${isPrimary ? 'text-sm' : 'text-xs'}`}>
                    {project.longDescription}
                </p>
                <div className="space-y-3 mt-auto">
                    <div className="flex flex-wrap gap-1.5">
                        {project.tags.slice(0, isPrimary ? 3 : 2).map(tag => (
                            <span key={tag} className={`px-2 py-1 bg-green-accent/5 text-green-accent rounded border border-green-accent/20 ${isPrimary ? 'text-xs' : 'text-[10px]'}`}>
                                {tag.replace(/-/g, ' ')}
                            </span>
                        ))}
                        {project.tags.length > (isPrimary ? 3 : 2) && (
                            <span className={`px-2 py-1 text-neutral-600 ${isPrimary ? 'text-xs' : 'text-[10px]'}`}>+{project.tags.length - (isPrimary ? 3 : 2)}</span>
                        )}
                    </div>
                    <div className="pt-3 border-t border-neutral-800/50 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            <a href={project.html_url} target="_blank" rel="noopener noreferrer" 
                               className={`cursor-target text-green-accent hover:text-green-accent/80 transition-colors flex items-center gap-1.5 ${isPrimary ? 'text-sm' : 'text-xs'}`}>
                                <GitHubIcon className={isPrimary ? 'w-3.5 h-3.5' : 'w-3 h-3'} />
                                GitHub
                            </a>
                            {project.liveUrl && (
                                <a href={project.liveUrl} target="_blank" rel="noopener noreferrer"
                                   className={`cursor-target text-green-accent hover:text-green-accent/80 transition-colors flex items-center gap-1.5 ${isPrimary ? 'text-sm' : 'text-xs'}`}>
                                    <svg className={isPrimary ? 'w-3.5 h-3.5' : 'w-3 h-3'} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"></path></svg>
                                    {isPrimary ? 'Try this out' : 'Live'}
                                </a>
                            )}
                        </div>
                        <ArrowRightIcon className={`text-neutral-700 ${isPrimary ? 'w-3.5 h-3.5' : 'w-3 h-3'}`} />
                    </div>
                </div>
            </div>
        </div>
...
const Projects: React.FC<{ projects: Project[] }> = ({ projects }) => {
    ...
    const sideProjects = projects.filter(p => !p.isPrimary);
    ...
    return (
        <AnimatedSection id="projects">
            <div className="max-w-5xl mx-auto px-6">
                ...
                {/* Primary Projects - Main Course */}
                <div className="mb-16">
                    <div className="flex items-center gap-3 mb-6">
                        <div className="w-2 h-2 bg-green-accent rounded-full animate-pulse"></div>
                        <h3 className="text-xl font-display font-semibold text-neutral-100 uppercase tracking-wider">
                            Flagship Projects
                        </h3>
                    </div>
